...
# ========================================================
# AUTO-INSTALLER: No external installation required!
# ========================================================
def setup_dependencies():
    try:
        from PIL import Image
    except ImportError:
        print("First time setup: Preparing skin renderer...")
...
class NarratorMCLauncher(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.accounts_frame, text="Accounts & Skin", font=ctk.CTkFont(size=22, weight="bold"), text_color="white").pack(anchor="w", padx=40, pady=(40, 10))
        ...
        self.left_panel = ctk.CTkFrame(self.acc_container, fg_color="transparent")
        self.left_panel.pack(side="left", fill="both", expand=True, pady=(0, 20))

        self.acc_card = ctk.CTkFrame(self.left_panel, fg_color=CARD_COLOR, corner_radius=10)
        ...
        self.username_entry = ctk.CTkEntry(self.acc_card, placeholder_text="Enter your username", width=300, height=40, fg_color=SIDEBAR_COLOR, border_color=ACCENT_CYAN)
        ...
        self.skin_card = ctk.CTkFrame(self.left_panel, fg_color=CARD_COLOR, corner_radius=10)
        self.skin_card.pack(fill="x")

        ctk.CTkLabel(self.skin_card, text="Custom Offline Skin (.png):", font=ctk.CTkFont(size=14)).pack(anchor="w", padx=20, pady=(20, 5))
        self.btn_select_skin = ctk.CTkButton(self.skin_card, text="Browse Skin", width=150, fg_color=SIDEBAR_COLOR, border_width=1, border_color=ACCENT_CYAN, hover_color="#1A2233", command=self.select_skin)
        self.btn_select_skin.pack(anchor="w", padx=20, pady=(5, 20))
        
        self.skin_path_var = ctk.StringVar(value=self.user_config.get("skin_path", ""))

        # Right Panel (Skin Preview Area)
        self.right_panel = ctk.CTkFrame(self.acc_container, fg_color=CARD_COLOR, corner_radius=10, width=220)
        self.right_panel.pack(side="right", fill="y", padx=(20, 0))
        self.right_panel.pack_propagate(False)

        ctk.CTkLabel(self.right_panel, text="Player Preview", font=ctk.CTkFont(size=14, weight="bold"), text_color=ACCENT_CYAN).pack(pady=(15, 0))
        
        self.preview_label = ctk.CTkLabel(self.right_panel, text="No Skin\nSelected", text_color=TEXT_MUTED, font=ctk.CTkFont(size=14))
        self.preview_label.pack(expand=True)

        if self.user_config.get("skin_path") and os.path.exists(self.user_config["skin_path"]):
            self.update_skin_preview(self.user_config["skin_path"])
        ...
        ctk.CTkLabel(self.versions_frame, text="Versions Manager\n(Development Mode)", font=ctk.CTkFont(size=20), text_color=TEXT_MUTED).pack(expand=True)

        self.show_home()

    # --- SKIN RENDERING LOGIC ---
    def update_skin_preview(self, skin_path):
        try:
            ...
            r_arm = skin.crop((44, 20, 48, 32))
            r_leg = skin.crop((4, 20, 8, 32))
            
            preview.paste(head, (4, 0))
            preview.paste(torso, (4, 8))
            preview.paste(r_arm, (0, 8))
            preview.paste(r_leg, (4, 20))
            
            if height == 64:
                l_arm = skin.crop((36, 52, 40, 64))
                l_leg = skin.crop((20, 52, 24, 64))
                preview.paste(l_arm, (12, 8))
                preview.paste(l_leg, (8, 20))
                
                # Second Layers (Jacket/Helmet)
                helmet = skin.crop((40, 8, 48, 16))
                jacket = skin.crop((20, 36, 28, 48))
                r_sleeve = skin.crop((44, 36, 48, 48))
                l_sleeve = skin.crop((52, 52, 56, 64))
                r_pants = skin.crop((4, 36, 8, 48))
                l_pants = skin.crop((4, 52, 8, 64))
                
                preview.paste(helmet, (4, 0), helmet)
                preview.paste(jacket, (4, 8), jacket)
                preview.paste(r_sleeve, (0, 8), r_sleeve)
                preview.paste(l_sleeve, (12, 8), l_sleeve)
                preview.paste(r_pants, (4, 20), r_pants)
                preview.paste(l_pants, (8, 20), l_pants)
            else:
            ...
            # Sharp resize for pixel art
            preview = preview.resize((140, 280), Image.NEAREST)
            img = ctk.CTkImage(light_image=preview, dark_image=preview, size=(140, 280))
    ...
    def select_skin(self):
        ...
        if filepath:
            ...
            self.save_config()
            self.update_skin_preview(filepath)
    ...
    def load_config(self):
        if os.path.exists(self.config_file):
            try:
                ...
                pass
        return {"ram": 4, "username": "", "skin_path": "", "enable_shaders": 0}
