...
CLIENT_ID = "YOUR_MICROSOFT_CLIENT_ID" # Optional: add your client id for MS login if you have one
...
class SupersonicClient(ctk.CTk):
    ...
    def check_for_updates(self):
        try:
            ...
            if res.status_code == 200:
                ...
                # You can add logic here to compare versions and prompt for update
        except: pass
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(links_f, text="🌐 Website", font=("Segoe UI", 11), text_color=TEXT_MUTED, cursor="hand2").pack(side="left")
        ctk.CTkLabel(links_f, text="💬 Discord", font=("Segoe UI", 11), text_color=ACCENT_BLUE, cursor="hand2").pack(side="right")
        ...
        ctk.CTkLabel(user_card, text=f"👑 Premium" if self.account_data.get("account_type") != "Offline" else "Offline", font=("Segoe UI", 11, "bold"), text_color="#FBBF24").pack(anchor="w", padx=15, pady=(0, 10))
    ...
    def render_dashboard(self):
        ...
        ctk.CTkLabel(tags_f, text=f"📦 Minecraft {self.settings.get('mc_version', '1.21.4')}", font=("Segoe UI", 11, "bold"), text_color=TEXT_PRIMARY, fg_color=BG_COLOR, corner_radius=5).pack(side="left", padx=(0, 10), ipadx=8, ipady=4)
        ...
        self.play_btn.pack()
        self.launch_status = ctk.CTkLabel(play_f, text=f"Latest Release ({self.settings.get('mc_version', '1.21.4')})", font=("Segoe UI", 11), text_color=TEXT_MUTED)
        ...
        ctk.CTkButton(add_head, text="📥 Install All", fg_color="transparent", border_width=1, border_color=BORDER_COLOR, text_color=TEXT_PRIMARY, width=100, command=lambda: self.switch_tab("addons")).pack(side="right")
        ...
        for i, (name, desc, icon) in enumerate(preview_mods):
            ...
            is_installed = os.path.exists(os.path.join(MODS_DIR, f"{name.lower().replace(' ', '')}.jar"))
            status_text = "✓ Installed" if is_installed else "Available"
            status_color = ACCENT_GREEN if is_installed else TEXT_MUTED
            ctk.CTkLabel(tf, text=status_text, font=("Segoe UI", 10, "bold"), text_color=status_color).pack(anchor="w")
        ...
        ctk.CTkButton(mp_head, text="Browse All", fg_color="transparent", border_width=1, text_color=TEXT_PRIMARY, width=100, command=lambda: self.switch_tab("modpacks")).pack(side="right")
    ...
    def build_agent_sidebar(self):
        ...
        ctk.CTkButton(self.right_panel, text="🛠️ Scan & Fix", fg_color="#4F46E5", hover_color="#4338CA", height=40, command=self.run_agent_fix).pack(fill="x", padx=20, pady=10)
    ...
    def run_agent_fix(self):
        # Placeholder for agent fix
        print("Agent is scanning and fixing...")
    ...
    def render_addons(self):
        ...
        for i, (slug, name, desc, icon) in enumerate(real_mods):
            ...
            is_installed = os.path.exists(os.path.join(MODS_DIR, f"{slug}.jar"))
            
            if is_installed:
                btn = ctk.CTkButton(card, text="✓ Installed", width=80, fg_color="transparent", border_width=1, border_color=ACCENT_GREEN, text_color=ACCENT_GREEN, state="disabled")
            else:
        ...
        def task():
            try:
                ...
                if res.status_code == 200 and res.json():
                    ...
                    filename = f"{slug}.jar" # use slug as filename for simplicity
    ...
    def render_settings(self):
        ...
        self.off_user.insert(0, self.account_data["username"] if self.account_data.get("account_type") == "Offline" else "Player")
        ...
        ctk.CTkButton(auth_f, text="Login Microsoft", fg_color=ACCENT_GREEN, hover_color="#059669", command=self.ms_login).pack(side="left")
        ...
        c6 = create_card(r2, "GRAPHICS (D3D12/VULKAN)")
        self.d3d12_switch = ctk.CTkSwitch(c6, text="Enable Direct3D 12 (Zink/ANGLE) wrapper", progress_color=ACCENT_BLUE, command=self.toggle_d3d12)
        if self.settings.get("d3d12_wrapper", True): self.d3d12_switch.select()
        self.d3d12_switch.pack(anchor="w", padx=15, pady=10)
        ctk.CTkLabel(c6, text="Forces OpenGL translation to D3D12 for better fps on Windows.", font=("Segoe UI", 10), text_color=TEXT_MUTED, justify="left").pack(anchor="w", padx=15, pady=(0, 15))
    ...
    def save_off_account(self):
        ...
        self.account_data["account_type"] = "Offline"
        self.save_account()
        self.lbl_acc_name.configure(text=self.account_data["username"])
        
    def toggle_d3d12(self):
        self.settings["d3d12_wrapper"] = bool(self.d3d12_switch.get())
        self.save_settings()

    def ms_login(self):
        # Implementation of Microsoft Login using minecraft_launcher_lib
        print("Initiating Microsoft OAuth Flow...")
        def task():
            try:
                # This opens the browser for login
                login_url, state, pkce = minecraft_launcher_lib.microsoft_account.get_secure_login_data(CLIENT_ID)
                webbrowser.open(login_url)
                
                # In a real app, you would start a local web server to catch the redirect callback
                # or ask the user to paste the URL. We mock the successful extraction for UI purposes here.
                # Since we don't have a redirect URI setup, this is a placeholder.
                
                self.thread_safe_update(self.lbl_acc_name, text="Microsoft User (Pending Auth)")
            except Exception as e:
                print(f"MS Login Error: {e}")
        threading.Thread(target=task, daemon=True).start()
    ...
    def game_launch_thread(self):
        ...
        def update_prog(max_val, cur_val):
            ...
            # FIX applied here: use a helper method to call .set() safely in main thread
            self.thread_safe_progress(self.launch_progress, val)
        ...
        try:
            ...
            # Note: Fabric installation logic could be added here
            # minecraft_launcher_lib.fabric.install_fabric(ver, MC_DIR)
        ...
        # D3D12 / OpenGL translation wrapper settings
        if self.settings.get("d3d12_wrapper", True):
            # This enables native translation over windows if supported, or via java properties
